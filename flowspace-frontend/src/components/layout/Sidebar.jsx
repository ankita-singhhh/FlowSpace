import { useState } from 'react'; // Updated to fix user reference
import { 
  LayoutDashboard, 
  CheckSquare, 
  Calendar, 
        ...
        <div className="flex items-center gap-3">
          ...
          {!isCollapsed && (
            <div className="flex-1 min-w-0">
              <p className={`text-sm font-medium ${isDark ? 'text-white' : 'text-gray-900'} truncate`}>{user?.name || 'User'}</p>
              <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'} truncate`}>{user?.email || 'user@example.com'}</p>
            </div>
          )}
        </div>
      </div>
